    ...
    private fun fetchStrategyCalculations(): List<StrategyCalculationRun> {
        ...
        return (0 until rows.length()).map { index ->
            ...
            StrategyCalculationRun(
                ...
                totalCostsToman = row.optDouble("total_costs_toman", 0.0),
                executionReserveRate = row.optDouble("execution_reserve_rate", 0.0),
                netProfitToman = row.getDouble("net_profit_toman"),
                realizedProfitToman = if (row.has("realized_profit_toman") && !row.isNull("realized_profit_toman")) row.getDouble("realized_profit_toman") else null,
    ...
    private fun fetchAssetSnapshot(): AssetSnapshot {
        ...
        val signals = if (signalRows == null) emptyList() else (0 until signalRows.length()).map { index ->
            ...
            AssetSignal(
                ...
                totalCostsToman = row.getDouble("total_costs_toman"),
                slippageToman = if (row.has("slippage_toman")) row.optDouble("slippage_toman") else null,
                netProfitToman = row.getDouble("net_profit_toman"),
                realizedProfitToman = if (row.has("realized_profit_toman") && !row.isNull("realized_profit_toman")) row.getDouble("realized_profit_toman") else null,
                minimumRequiredProfitToman = row.getDouble("minimum_required_profit_toman"),
